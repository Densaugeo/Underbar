(function() {
  ...
  var filter = function(collection, test, notReject = true) {
    var arr = [];
    
    for (var i = 0; i < collection.length; i++) {
      
      if (notReject && test(collection[i])) {
        arr.push(collection[i]); // filter is true
      
      } else if (!notReject && !test(collection[i])) { 
        arr.push(collection[i]); // reject is true
      };
    }
  ...
  _.reject = function(collection, test) {
    return filter(collection, test, false);
  };
